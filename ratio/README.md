...
python3 ratio/build_adjudicated.py  # second scenario: API labels for ambiguous records
python3 -m http.server 8010       # then open http://localhost:8010/ratio/
```

The page has two scenarios: the rules build (`output/aggregates.json` +
`labels.jsonl`) and the adjudicated build (`?scenario=adjudicated`, reading
`aggregates_adjudicated.json` + `labels_adjudicated.jsonl`). The adjudicated
files are generated by `build_adjudicated.py` from
`output/adjudicated.jsonl`; the rules outputs are never overwritten.
Adjudications come from `ratio/adjudicate.py` (API, cached under
`data/llm_cache/`, quotes machine-verified by `verify_adjudication.py`).
...
### v2 result (census corpus, rules v2 + expanded lexicon)

Rules v2 ([V2_PLAN.md](V2_PLAN.md)) adds 13 mined keywords, conditional
voters for anchored social vocabulary, a relative-margin + evidence-floor
decision and three near-tie boundary rules. Validated against the
senior-expert labels on the atlas (`python3 ratio/eval_rules.py`): agreement
rose 53.3% -> **56.7%** and the rules-ambiguous count fell 770 -> 644, with
no category's agreement dropping. `EVIDENCE_FLOOR` was tuned once on that
set (0.70 rejected: it raised ambiguity to 916 with no accuracy gain). A
50-record SE model-adjudication pilot then exposed relevance-gate leaks
(the NMR "mouse" tripping the biomedical guard, battery "cell" in
BIO_CONTEXT, "cohort of specialists", antifouling chemistry) and social
vocabulary gaps; the gate fixes and two conditional voters (`housing`,
`social inequalities`) are included below.

| region | ratio v1 | ratio v2 | ambiguous v1 | ambiguous v2 | grants |
| --- | --- | --- | --- | --- | --- |
| SE | 18.6% | 22.8% | 30.7% | 19.8% | 870 |
| EU | 64.0% | 64.8% | 60.8% | 53.0% | 239 |
| US | 42.8% | 49.5% | 18.7% | 8.4% | 12,115 |

Label distribution v2: 6,531 fundamental_aging, 4,672 age_related_disease,
1,381 ambiguous, 245 not_relevant, 176 social_population_aging, 166 care,
53 intervention. Every ratio moved up because ambiguous money moved back
into the denominator and, mostly, the numerator; the US ratio lands near the
senior-expert headline (49.9%), but those run on different corpora (census
vs atlas sample) and are not directly comparable. SE stays limited by the
anchored social vocabulary; Phase 2 (model adjudication) targets what is
still ambiguous. The v1 outputs remain in git history.

Phase 2 pilot (2026-09-12): the 50 largest SE ambiguous grants were
adjudicated by deepseek-v4.1-flash in-session against the rubric, one
verbatim quote per record, all quotes machine-verified
(`ratio/verify_adjudication.py`; 50/50). 49/50 were resolved: 19
not_relevant (mostly engineering the gate then leaked), 14 social, 8
fundamental, 4 care, 2 intervention, 2 disease, 1 still ambiguous. On that
40/50 adversarial subset the updated rules agree with 15 and still park 36
(mostly the social and fundamental calls) as ambiguous — the evidence that
the remaining gap is adjudication, not keywords. Adjudications:
`output/adjudicated_se_pilot.jsonl`.

### Phase 2 result (API adjudication, deepseek-flash)

`ratio/adjudicate.py` sent every remaining ambiguous record (SE 172, EU 72,
US 1,070) to `deepseek-flash` with the same rubric, temperature 0, one
verbatim quote per record; 1,314/1,314 quotes machine-verified and 0
failures. A 29-record overlap between the API pass and the in-session pass
agreed on 24 (83%); the disagreements were boundary calls (frailty biology
vs clinical, metformin mechanism vs intervention, wellness anthropology).
Replacing the ambiguous labels with the adjudicated ones:

| region | ratio rules v2 | ratio adjudicated | ambiguous share v2 | ambiguous share adjudicated |
| --- | --- | --- | --- | --- |
| SE | 0.228 | 0.240 | 19.8% | ~0.0% (1 record) |
| EU | 0.648 | 0.684 | 53.0% | 0.2% |
| US | 0.495 | 0.500 | 8.4% | 0.2% |

The US adjudicated ratio (50.0%) lands on the senior-expert re-tag figure
(49.9%) computed on the atlas sample — different corpora, but the
convergence is the strongest evidence so far that the headline is not an
artifact of one labelling scheme. SE remains the lowest region. Candidate
labels are one model pass, not human ground truth; the 30-record human
benchmark is still the missing instrument. Adjudications:
`output/adjudicated.jsonl`; API cache under `data/llm_cache/` (gitignored).
...
- [`output/adjudicated.jsonl`](output/adjudicated.jsonl) — the model
  adjudications (1,314 records, deepseek-flash, quotes machine-verified);
  [`adjudicate.py`](adjudicate.py) reproduces them through the API,
  [`verify_adjudication.py`](verify_adjudication.py) checks quotes.
- [`output/aggregates_adjudicated.json`](output/aggregates_adjudicated.json)
  and [`output/labels_adjudicated.jsonl`](output/labels_adjudicated.jsonl) —
  the adjudicated scenario built by [`build_adjudicated.py`](build_adjudicated.py);
  the page shows them at `?scenario=adjudicated`.
